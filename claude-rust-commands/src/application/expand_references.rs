...
pub fn expand_message_content(input: &str) -> Vec<claude_rust_types::ContentBlock> {
    use base64::Engine;

    let mut text = input.to_string();
    let mut text_file_blocks: Vec<String> = Vec::new();
    let mut image_blocks: Vec<claude_rust_types::ContentBlock> = Vec::new();
    ...
    for cap in FILE_REF_RE.captures_iter(input) {
        ...
        if IMAGE_EXTS.contains(&ext.as_str()) {
            ...
            image_blocks.push(claude_rust_types::ContentBlock::Image {
    ...
    let mut result: Vec<claude_rust_types::ContentBlock> =
        vec![claude_rust_types::ContentBlock::Text { text }];
